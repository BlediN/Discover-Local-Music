...
# 3. Create the database table and seed data
...
- [x] **The user can search for items by a specific attribute (venue)**
...
2. **Configure Environment Variables:**
   ...
   ```
   DATABASE_URL=postgres://user:password@localhost:5432/discover_music
...
3. **Initialize the PostgreSQL Table:**
   ```bash
   psql "$DATABASE_URL" -f db/schema.sql
   ```
